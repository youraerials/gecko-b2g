...
GonkRecorder::WrappedMediaSource::WrappedMediaSource(
    const sp<MediaSource>& encoder)
    : mEncoder(encoder),
...
status_t GonkRecorder::WrappedMediaSource::start(MetaData* params) {
...
sp<MetaData> GonkRecorder::WrappedMediaSource::getFormat() {
...
// If seconds >  0, it is the time spacing (seconds) between 2 neighboring I
// frames
...
status_t GonkRecorder::setParamMovieTimeScale(int32_t timeScale) {
  ...
  if (timeScale < 600 || timeScale > 96000) {
    RE_LOGE("Time scale (%d) for movie is out of range [600, 96000]",
            timeScale);
    return BAD_VALUE;
  }
...
status_t GonkRecorder::setParamVideoTimeScale(int32_t timeScale) {
  ...
  if (timeScale < 600 || timeScale > 60000) {
    RE_LOGE("Time scale (%d) for video is out of range [600, 60000]",
            timeScale);
    return BAD_VALUE;
  }
...
status_t GonkRecorder::setParamAudioTimeScale(int32_t timeScale) {
  ...
  if (timeScale < 600 || timeScale > 96000) {
    RE_LOGE("Time scale (%d) for audio is out of range [600, 96000]",
            timeScale);
    return BAD_VALUE;
  }
...
status_t GonkRecorder::setParamGeoDataLongitude(int64_t longitudex10000) {
...
status_t GonkRecorder::setParamGeoDataLatitude(int64_t latitudex10000) {
...
status_t GonkRecorder::prepare() {
  if (mVideoSource != VIDEO_SOURCE_LIST_END &&
      mVideoEncoder != VIDEO_ENCODER_LIST_END && mVideoHeight &&
      mVideoWidth &&  // Video recording
      (mVideoHeight * mVideoWidth >= RES_720P)) {
    // TODO: Above check needs to be updated when mMaxFileDurationUs is set from
    // camera app
...
sp<MediaSource> GonkRecorder::createAudioSource() {
  sp<AudioSource> audioSource =
      new AudioSource(mAudioSource, mSampleRate, mAudioChannels);
...
#  endif
  ...
  sp<MediaSource> audioEncoder = OMXCodec::Create(
      client.interface(), encMeta, true /* createEncoder */, audioSource);
  mAudioSourceNode = audioSource;
...
status_t GonkRecorder::startAMRRecording() {
  ...
  if (mOutputFormat == OUTPUT_FORMAT_AMR_NB) {
    if (mAudioEncoder != AUDIO_ENCODER_DEFAULT &&
        mAudioEncoder != AUDIO_ENCODER_AMR_NB) {
      RE_LOGE("Invalid encoder %d used for AMRNB recording", mAudioEncoder);
      return BAD_VALUE;
    }
  } else {  // mOutputFormat must be OUTPUT_FORMAT_AMR_WB
    if (mAudioEncoder != AUDIO_ENCODER_AMR_WB) {
      RE_LOGE("Invlaid encoder %d used for AMRWB recording", mAudioEncoder);
      return BAD_VALUE;
    }
  }
...
status_t GonkRecorder::startRTPRecording() { return INVALID_OPERATION; }
...
void GonkRecorder::clipVideoFrameRate() {
  ...
  if (mFrameRate < minFrameRate && minFrameRate != -1) {
    RE_LOGW(
        "Intended video encoding frame rate (%d fps) is too small"
        " and will be set to (%d fps)",
        mFrameRate, minFrameRate);
    mFrameRate = minFrameRate;
  } else if (mFrameRate > maxFrameRate && maxFrameRate != -1) {
    RE_LOGW(
        "Intended video encoding frame rate (%d fps) is too large"
        " and will be set to (%d fps)",
...
void GonkRecorder::clipVideoBitRate() {
  ...
  if (mVideoBitRate < minBitRate && minBitRate != -1) {
    RE_LOGW(
        "Intended video encoding bit rate (%d bps) is too small"
        " and will be set to (%d bps)",
        mVideoBitRate, minBitRate);
    mVideoBitRate = minBitRate;
  } else if (mVideoBitRate > maxBitRate && maxBitRate != -1) {
    RE_LOGW(
        "Intended video encoding bit rate (%d bps) is too large"
        " and will be set to (%d bps)",
...
void GonkRecorder::clipVideoFrameWidth() {
  ...
  if (mVideoWidth < minFrameWidth && minFrameWidth != -1) {
    RE_LOGW(
        "Intended video encoding frame width (%d) is too small"
        " and will be set to (%d)",
        mVideoWidth, minFrameWidth);
    mVideoWidth = minFrameWidth;
  } else if (mVideoWidth > maxFrameWidth && maxFrameWidth != -1) {
    RE_LOGW(
        "Intended video encoding frame width (%d) is too large"
        " and will be set to (%d)",
...
void GonkRecorder::setDefaultProfileIfNecessary() {
  ...
  if (durationUs == mMaxFileDurationUs && fileFormat == mOutputFormat &&
      videoCodec == mVideoEncoder && videoBitRate == mVideoBitRate &&
      videoFrameRate == mFrameRate && videoFrameWidth == mVideoWidth &&
      videoFrameHeight == mVideoHeight && audioCodec == mAudioEncoder &&
      audioBitRate == mAudioBitRate && audioSampleRate == mSampleRate &&
...
void GonkRecorder::clipAudioBitRate() {
  ...
  int minAudioBitRate = mEncoderProfiles->getAudioEncoderParamByName(
      "enc.aud.bps.min", mAudioEncoder);
  if (minAudioBitRate != -1 && mAudioBitRate < minAudioBitRate) {
    RE_LOGW(
        "Intended audio encoding bit rate (%d) is too small"
        " and will be set to (%d)",
        mAudioBitRate, minAudioBitRate);
    mAudioBitRate = minAudioBitRate;
  }

  int maxAudioBitRate = mEncoderProfiles->getAudioEncoderParamByName(
      "enc.aud.bps.max", mAudioEncoder);
  if (maxAudioBitRate != -1 && mAudioBitRate > maxAudioBitRate) {
    RE_LOGW(
        "Intended audio encoding bit rate (%d) is too large"
        " and will be set to (%d)",
...
void GonkRecorder::clipAudioSampleRate() {
  ...
  if (minSampleRate != -1 && mSampleRate < minSampleRate) {
    RE_LOGW(
        "Intended audio sample rate (%d) is too small"
        " and will be set to (%d)",
  ...
  if (maxSampleRate != -1 && mSampleRate > maxSampleRate) {
    RE_LOGW(
        "Intended audio sample rate (%d) is too large"
        " and will be set to (%d)",
...
void GonkRecorder::clipNumberOfAudioChannels() {
  ...
  int minChannels = mEncoderProfiles->getAudioEncoderParamByName(
      "enc.aud.ch.min", mAudioEncoder);
  if (minChannels != -1 && mAudioChannels < minChannels) {
    RE_LOGW(
        "Intended number of audio channels (%d) is too small"
        " and will be set to (%d)",
        mAudioChannels, minChannels);
    mAudioChannels = minChannels;
  }

  int maxChannels = mEncoderProfiles->getAudioEncoderParamByName(
      "enc.aud.ch.max", mAudioEncoder);
  if (maxChannels != -1 && mAudioChannels > maxChannels) {
    RE_LOGW(
        "Intended number of audio channels (%d) is too large"
        " and will be set to (%d)",
...
void GonkRecorder::clipVideoFrameHeight() {
  ...
  if (minFrameHeight != -1 && mVideoHeight < minFrameHeight) {
    RE_LOGW(
        "Intended video encoding frame height (%d) is too small"
        " and will be set to (%d)",
        mVideoHeight, minFrameHeight);
    mVideoHeight = minFrameHeight;
  } else if (maxFrameHeight != -1 && mVideoHeight > maxFrameHeight) {
    RE_LOGW(
        "Intended video encoding frame height (%d) is too large"
        " and will be set to (%d)",
...
status_t GonkRecorder::setupMediaSource(sp<MediaSource>* mediaSource) {
...
status_t GonkRecorder::setupCameraSource(sp<GonkCameraSource>* cameraSource) {
  status_t err = OK;
  ...
  if (property_get("debug.camcorder.disablemeta", value, NULL) && atoi(value)) {
    useMeta = false;
  }

  *cameraSource =
      GonkCameraSource::Create(mCameraHw, videoSize, mFrameRate, useMeta);
  ...
  if (mFrameRate == -1) {
    int32_t frameRate = 0;
    CHECK((*cameraSource)->getFormat()->findInt32(kKeyFrameRate, &frameRate));
    RE_LOGI(
...
#if defined(MOZ_WIDGET_GONK)
status_t GonkRecorder::setupVideoEncoder(sp<MediaSource> cameraSource,
...
status_t GonkRecorder::setupVideoEncoder(sp<MediaSource> cameraSource,
  ...
  sp<MediaSource> encoder =
      OMXCodec::Create(client.interface(), enc_meta, true /* createEncoder */,
                       cameraSource, NULL, encoder_flags);
  if (encoder == NULL) {
...
status_t GonkRecorder::setupMPEG4Recording(
    int outputFd, int32_t videoWidth, int32_t videoHeight, int32_t videoBitRate,
    int32_t* totalBitRate, sp<MediaWriter>* mediaWriter,
  ...
  *totalBitRate = 0;
  ...
  if (mVideoSource < VIDEO_SOURCE_LIST_END) {
    ...
    sp<GonkCameraSource> cameraSource =
        reinterpret_cast<GonkCameraSource*>(mediaSource.get());
...
void GonkRecorder::setupMPEG4MetaData(int64_t startTimeUs, int32_t totalBitRate,
  ...
  char value[PROPERTY_VALUE_MAX];
  if (property_get("debug.camcorder.rotation", value, 0) > 0 &&
      atoi(value) >= 0) {
...
status_t GonkRecorder::startMPEG4Recording() {
  ...
  status_t err =
      setupMPEG4Recording(mOutputFd, mVideoWidth, mVideoHeight, mVideoBitRate,
                          &totalBitRate, &mWriter, &mWriterSource);
  if (err != OK) {
    return err;
  }
...
status_t GonkRecorder::dump(int fd, const Vector<String16>& args) const {
  RE_LOGV("dump");
